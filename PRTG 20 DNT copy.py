...
columns_to_drop = ['ID(RAW)', 'Date Time(RAW)', 'Parent(RAW)', 'Type(RAW)', 'Object(RAW)', 'Status(RAW)', 'Message(RAW)']
df.drop(columns=columns_to_drop, inplace=True)
df.to_csv(csv_path, index=False)
print("CSV data processed successfully.")

# Separate entries with "returned no data" in the message
no_data_df = df[df['Message'].str.contains("returned no data", na=False)]
regular_df = df[~df['Message'].str.contains("returned no data", na=False)]

status_counts = regular_df['Status'].value_counts()
...
for _, row in no_data_df.iterrows():
    ...
    message = row.get('Message', '')
    sensid = row.get('ID')
    data_by_no_data[status][type_].append((date_time, object_, parent, message, sensid))
        ...
        for parent, grouped_items in grouped_objects.items():
            ...
            for obj, grouped_items_by_type in grouped_objects_by_type.items():
                if grouped_items_by_type:
                    ...
                    sensids = [item[2] for item in grouped_items_by_type if len(item) >= 3]
                else:
                    sensid = ''
                    sensids = []
...
# Add No Data section
for status, types in data_by_no_data.items():
    html_content += f"<li onclick='toggleStatus(event)' class='status-no-data'>{status} <strong> ({total_types_by_no_data[status]})</strong><ul class='hidden'>"
