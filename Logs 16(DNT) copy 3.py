    ...
    for type_, objects in types.items():
        ...
        for parent, grouped_items in grouped_objects.items():
            ...
            for obj, grouped_items_by_type in grouped_objects_by_type.items():
                # Gather sensids for this object
                sensids = [sensid for _, _, sensid in grouped_items_by_type]
                # Choose the first sensid as the sensid for this object
                chosen_sensid = sensids[0] if sensids else ''
                if "Group" in type_:
                    sensor_url = f"https://{server_address}/group.htm?id={chosen_sensid}"
                elif "Device" in type_:
                    sensor_url = f"https://{server_address}/device.htm?id={chosen_sensid}"
                elif "Probe" in type_:
                    sensor_url = f"https://{server_address}/probenode.htm?id={chosen_sensid}"
                elif "Notification Template" in type_:
                    sensor_url = f"https://{server_address}/editnotification.htm?id={chosen_sensid}"
                else:
                    sensor_url = f"https://{server_address}/sensor.htm?id={chosen_sensid}"

                html_content += f"<li><strong>ID:<a href='{sensor_url}'> {chosen_sensid} </a>Sensor:</strong> {obj} <strong>({len(grouped_items_by_type)})</strong><ul class='hidden'>"
