...
for status, types in data_by_status.items():
    html_content += f"<li onclick='toggleChildren(this)'>{status} ({len(types)})<ul class='hidden'>"
    for type_, objects in types.items():
        html_content += f"<li onclick='toggleChildren(this)'>{type_} ({len(objects)})<ul class='hidden'>"
...
# Serve HTML using Python's built-in HTTP server
